...
class _UploadScreenState extends State<UploadScreen> {
  ...
  Future<void> _uploadFile() async {
    ...
    try {
      // Save metadata only (no file bytes are uploaded to Firebase Storage).
      await _uploadService.saveDocument(
        title: _titleController.text.trim(),
        fileName: _selectedFile?.name ?? 'No file attached',
        fileSize: _selectedFile?.size ?? 0,
        validityDate: _validityDate!,
      );

      // Trigger validity check to create an expiry reminder if needed.
      await _uploadService.checkValidityExpiry();
      ...
      if (mounted) {
        // Requested behavior: always show a dummy success message.
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('File uploaded successfully!'),
            backgroundColor: Colors.green.shade400,
